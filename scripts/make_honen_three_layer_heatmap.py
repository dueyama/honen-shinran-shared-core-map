#!/usr/bin/env python3
"""Build a three-layer Honen chunk-sequence heatmap.

Rows are semantic affinity, style/lexical groups, and source-marker groups.
Columns are Honen safe chunks in reading order. This script reads local-only
...
from make_shinran_three_layer_heatmap import (  # noqa: E402
    FONT_CANDIDATES,
    PANEL_COLORS,
    SOURCE_MARKER_ROWS,
)
...
MAX_TOKENS = 700
OVERLAP = 100
OUT_JSON = ROOT / "data/outputs/honen_three_layer_sequence_2026-06-04_text-embedding-3-large_700_100.json"
OUT_CSV = ROOT / "data/outputs/honen_three_layer_sequence_2026-06-04.csv"
FIGURE_PATH = ROOT / "docs/figures/honen-three-layer-sequence-heatmap.png"
DOC_PATH = ROOT / "docs/honen-three-layer-sequence-2026-06-04.md"


SEMANTIC_ROWS = ["親鸞", "龍樹", "天親", "曇鸞", "道綽", "善導", "源信"]
...
def make_layers() -> dict[str, Any]:
    ...
    honen_chunks = [chunks[index] for index in honen_indices]
    honen_vectors = matrix[np.array(honen_indices, dtype=int)]

    semantic_scores = np.zeros((len(SEMANTIC_ROWS), len(honen_chunks)), dtype=float)
    ...
    for row_index, author in enumerate(SEMANTIC_ROWS):
        ...
        sims = honen_vectors @ ref_matrix.T
    ...
    style_counts = np.zeros((len(STYLE_ROWS), len(honen_chunks)), dtype=float)
    marker_row_names = list(SOURCE_MARKER_ROWS.keys())
    marker_counts = np.zeros((len(marker_row_names), len(honen_chunks)), dtype=float)
    chunk_rows = []

    for col, chunk in enumerate(honen_chunks):
        ...
        chunk_rows.append(
            {
                ...
                "section_label": range_label(chunk.line_start, HONEN_SECTION_RANGES, "未分類"),
...
def section_boundaries(chunk_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out = []
    current = None
    start = 0
...
def draw_heatmap(layers: dict[str, Any]) -> None:
    ...
    chart_x = label_w
    y = top_h
    boundaries = section_boundaries(chunk_rows)
    ...
    y += gap
    draw_panel(y, "典拠マーカー層: 明示マーカー", marker_row_names, layers["marker_scaled"], PANEL_COLORS["marker"])
...
def write_outputs(layers: dict[str, Any]) -> None:
    ...
    summary = {
        ...
        "method": {
            "target": "Honen T2608 chunks only, in chunk_index order",
            "embedding_model": MODEL,
            "embedding_dimension": 3072,
            "chunking": f"Unicode-safe {MAX_TOKENS}-token chunks, {OVERLAP}-token overlap",
            "semantic_layer": "max cosine from each Honen chunk to chunks in each reference author group, computed in 3072D",
        ...
        "summaries": {
            "semantic_top_by_section": count_by(chunk_rows, "section_label", "semantic_top_group"),
            "style_top_by_section": count_by(chunk_rows, "section_label", "style_top_group"),
            "source_marker_top_by_section": count_by(chunk_rows, "section_label", "source_marker_top_group"),
        },
        "chunks": [
            {
    ...
    with OUT_CSV.open("w", encoding="utf-8", newline="") as handle:
        fields = ["chunk_index", "section_label", "line_start", "line_end", "layer", "feature", "value", "text_sha256", "replacement_chars"]
...
def render_doc(summary: dict[str, Any]) -> str:
    lines: list[str] = [
        ...
        "## 意味層: 章別トップ近傍",
        "",
        *md_table(summary["summaries"]["semantic_top_by_section"], [("位置", "section_label"), ("top", "semantic_top_group"), ("chunks", "chunk_count")]),
        "",
        "## 文体語彙層: 章別トップ語群",
        "",
        *md_table(summary["summaries"]["style_top_by_section"], [("位置", "section_label"), ("top", "style_top_group"), ("chunks", "chunk_count")]),
        "",
        "## 典拠マーカー層: 章別トップマーカー",
        "",
